#!/usr/bin/env bash
# Script to quickly grab current Master Quartz EXP and EXP until next level in
# The Legend of Heroes: Trails of Cold Steel. Once these EXP values
# are captured, they are added together and then subtracted by 1, so I
# could plug that value into a memory editor (gameconqueror) to raise my Master Quartz levels
...
TEXT_OUTPUT=$(maim -g 96x53+2871+485 | convert +dither -colors 3 -colors 2 -colorspace gray -normalize -scale 300% -channel RGB -negate - - | tesseract --dpi 300 - - | perl -pe 's/\n/ /g' | awk '{print $1+$2-1}')

# Output value to screen as a notification and send to clipboard
notify-send "$TEXT_OUTPUT"
echo -n "$TEXT_OUTPUT" | xclip -selection c
